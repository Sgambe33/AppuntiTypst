...
#proposition()[
  $L$ è decidibile $=> L$ è enumerabile.
]
...
#observation()[
  In generale, l'implicazione inversa *non* vale: $L$ enumerabile $arrow.r.double.not L$ decidibile. Vale però il risultato seguente, in cui si chiede l'enumerabilità anche del complementare $L^c = Sigma^* \\ L$.
]
...
Vediamo ora i due "costruttori" di funzioni, a partire dalle iniziali.
...
#example("Funzione differenza troncata")[
  ...
  $
    cases(
      ...
      x minus.dot (y+1) = (nu compose epsilon_3^((3)))(x, y, x minus.dot y)
    )
  $
]
...
#example("Funzione distanza")[
  ...
  $
  Uno dei due addendi dà il risultato voluto a seconda che $x <= y$ oppure $x > y$, mentre l'altro vale 0:
  $
...
#proof()[
  ...
  $
    g(arrow(x), 0) &= sum_(y=0)^0 f(arrow(x), y) = f(arrow(x), 0) = (f compose (epsilon_1^((k)), dots, epsilon_k^((k)), C_0^((k))))(arrow(x)) \
